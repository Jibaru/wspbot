/**
 * The Crafter Station mark.
 *
 * The genuine artwork, taken from the brand system at brand.crafter.run: one interlaced glyph on
 * a 258×258 grid built from four R=64 circles. It is reproduced rather than redrawn on purpose —
 * the brand's own list of forbidden things ends with "replace with similar marks", and an
 * approximation is exactly that.
 *
 * Two of the five variants are used here. `filled` is the primary, with the forged-gold gradient
 * the system specifies (#FFD435 → #FFC107 → #C68B05). `monoline` is for inline UI, where a
 * filled glyph at 20px turns into a blob.
 *
 * The gradient needs an `id`, and duplicate ids in one document are invalid, so callers pass one
 * rather than the component inventing it — which keeps this a server component.
 */
...
export function CrafterMark({
  id,
  size = 24,
  variant = "filled",
  className,
}: {
  /** Unique within the page; it names the gradient. */
  id: string;
  size?: number;
  variant?: "filled" | "monoline";
  className?: string;
}) {
  const gradient = `cs-grad-${id}`;
  ...
  return (
    <svg
      ...
      width={size}
      height={size}
      // Never stretched or skewed, which the brand forbids outright.
      ...
      {variant === "filled" && (
        <defs>
          <linearGradient id={gradient} x1="0.15" y1="0" x2="0.85" y2="1">
            <stop offset="0%" stopColor="#FFD435" />
            <stop offset="55%" stopColor="#FFC107" />
            <stop offset="100%" stopColor="#C68B05" />
          </linearGradient>
        </defs>
      )}
      <path
        d={MARK}
        fill={variant === "filled" ? `url(#${gradient})` : "none"}
        stroke={variant === "monoline" ? "currentColor" : "none"}
        strokeWidth={variant === "monoline" ? 10 : undefined}
        strokeLinejoin={variant === "monoline" ? "round" : undefined}
      />
    </svg>
  );
}
...
/**
 * The horizontal lockup — mark plus wordmark, the primary one, for headers and footers.
 *
 * Spacing follows the system: the mark stands at the wordmark's cap height, with a gap of
 * R/2 = 32 units on a 258 grid, which is 0.124 of the mark's rendered size. The dot is forged
 * gold, always.
 */
export function CrafterLockup({ id, size = 20 }: { id: string; size?: number }) {
  return (
    <span
      style={{
        display: "inline-flex",
    ...
    >
      <CrafterMark id={id} size={size} />
      <span>
        Crafter Station<span className="lp-dot">.</span>
      </span>
    </span>
  );
}
